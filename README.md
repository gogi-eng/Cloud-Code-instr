...
4. **Хуки (hooks)** — автоматизации, запускаемые по событиям (см. [ниже](#хуки-hooks)).
...
| Хук | Что делает | Кому нужен |
|-----|-----------|------------|
| 🚀 **boot-md** | При старте gateway выполняет инструкции из файла `BOOT.md` в вашем workspace. Подходит для автоматической инициализации (загрузка данных, приветствие, проверки). | Продвинутым пользователям, которые хотят автоматизировать действия при запуске |
| 📎 **bootstrap-extra-files** | Подгружает дополнительные файлы (например, `AGENTS.md`, `TOOLS.md`) в контекст агента при сборке промпта. Файлы не копируются — внедряются в память (in-memory). Полезно для монорепозиториев с несколькими конфигами. | Разработчикам с крупными проектами или несколькими агентами |
| 📝 **command-logger** | Записывает все выполненные команды в лог-файл `~/.openclaw/logs/commands.log` в формате JSONL. Полезно для аудита и отладки. | Всем, кто хочет отслеживать действия агента |
| 💾 **session-memory** | При командах `/new` или `/reset` автоматически сохраняет контекст текущей сессии в файл `<workspace>/memory/YYYY-MM-DD-slug.md`. Имя файла генерируется LLM на основе содержания разговора. | Всем, кто хочет, чтобы агент «помнил» прошлые разговоры |

### Рекомендации для начинающих

Если не уверены — выберите **«Skip for now»**. Хуки можно включить в любой момент:

```bash
# Посмотреть доступные хуки
openclaw hooks list

# Узнать подробности о конкретном хуке
...
### Какие хуки стоит включить?

- **session-memory** — рекомендуется большинству пользователей. Агент будет запоминать контекст между сессиями.
- **command-logger** — полезен для отладки и если вы хотите видеть, какие команды агент выполнял.
- **boot-md** и **bootstrap-extra-files** — для продвинутых сценариев. Включайте, когда разберётесь с базовой настройкой.
...
| `openclaw hooks list` | Список доступных хуков |
| `openclaw hooks enable <name>` | Включить хук |
| `openclaw hooks disable <name>` | Отключить хук |
